        ...
        for row in image:
            for pixel in row:
                ...
                if data_index >= data_len:
                    break
    return image
def decode(image_name, n_bits=3, in_bytes=False):
    print("[+] Decoding...")
    # read the image
    image = cv2.imread(image_name)
    binary_data = ""
...
#             decoded_data = decode("./website/static/uploads/updatedFiles/data_decoded_encoded.PNG", n_bits=args.n_bits, in_bytes=True)
...
decoded_data = decode("./website/static/uploads/updatedFiles/data_decoded_encoded.PNG", 3, in_bytes=True)
